    ...
    <div className="space-y-6 max-w-4xl mx-auto">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Pusat pemberitahuan aktivitas aplikasi dan pesanan Anda.</p>
        </div>
        <button 
      ...
      <div className="bg-background rounded-2xl border border-foreground/10 shadow-sm overflow-hidden">
        {notifications.length > 0 ? (
          <div className="divide-y divide-foreground/5">
            {notifications.map((notification) => (
              <div 
                ...
                className={`p-5 flex gap-4 transition-colors hover:bg-muted/50 cursor-pointer ${!notification.isRead ? 'bg-primary/5' : ''}`}
              >
                <div className="mt-1 shrink-0">
                  <div className="w-10 h-10 rounded-full bg-background border border-foreground/10 flex items-center justify-center shadow-sm">
                    {getIconForType(notification.type)}
                  </div>
                </div>
                <div className="flex-1 space-y-1">
                  <div className="flex justify-between items-start">
                    <h3 className={`text-base ${!notification.isRead ? 'font-semibold text-foreground' : 'font-medium text-foreground/80'}`}>
                      {notification.title}
                    </h3>
          ...
          <div className="p-12 text-center">
            ...
            <h3 className="text-lg font-medium text-foreground mb-1">Belum ada notifikasi</h3>
            <p className="text-muted-foreground text-sm">Aktivitas pesanan baru Anda akan muncul di sini.</p>
          </div>
        )}
      </div>
    </div>
