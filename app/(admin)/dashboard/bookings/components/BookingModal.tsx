        ...
        >
          ...
          <div className="flex items-center justify-between p-6 border-b border-foreground/10">
            <div>
              ...
              <p className="text-muted-foreground font-mono mt-1 text-sm">{booking.customCode || `#${booking.id}`}</p>
            </div>
            <button
              onClick={handleClose}
          ...
          <div className="flex-1 overflow-y-auto p-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              ...
              <div className="space-y-6">
                <div>
                  <h3 className="text-sm font-semibold text-muted-foreground uppercase tracking-wider mb-3">Informasi Klien</h3>
                  <div className="space-y-3 bg-muted/30 p-4 rounded-2xl border border-foreground/5">
                          ...
                          {booking.instagram ? (
                            <>
                              ...
                              <button
                                ...
                                className="p-1.5 bg-muted hover:bg-muted-foreground/10 text-muted-foreground rounded-md transition-colors"
                ...
                <div>
                  <h3 className="text-sm font-semibold text-muted-foreground uppercase tracking-wider mb-3">Detail Acara</h3>
                  <div className="space-y-3 bg-muted/30 p-4 rounded-2xl border border-foreground/5">
                    <div className="flex items-center gap-3">
                      <Info className="w-5 h-5 text-primary" />
                      <div>
                        <p className="text-sm text-muted-foreground">Nama Acara</p>
              ...
              <div className="space-y-6">
                <div>
                  <h3 className="text-sm font-semibold text-muted-foreground uppercase tracking-wider mb-3">Layanan</h3>
                ...
                <div>
                  <h3 className="text-sm font-semibold text-muted-foreground uppercase tracking-wider mb-3">Pembayaran</h3>
                  <div className="space-y-4 bg-muted/30 p-4 rounded-2xl border border-foreground/5">
                    ...
                    <div className="flex justify-between items-center">
                      ...
                      <p className="font-medium text-primary">{formatCurrency(booking.dpAmount)}</p>
                    </div>
                    <div className="border-t border-foreground/10 pt-3 flex items-start gap-3">
                      <Clock className="w-5 h-5 text-amber-500 shrink-0" />
                      <div>
                        <p className="text-sm text-muted-foreground">Tenggat Waktu Bayar DP</p>
                ...
                {booking.notes && (
                  <div>
                    <h3 className="text-sm font-semibold text-muted-foreground uppercase tracking-wider mb-2">Catatan Tambahan</h3>
                    <p className="text-sm bg-muted/30 p-4 rounded-xl border border-foreground/5 italic">
                      "{booking.notes}"
                    </p>
                  </div>
                )}
          ...
          {booking.status !== 'COMPLETED' && booking.status !== 'CANCELED' && (
            <div className="p-6 border-t border-foreground/10 flex justify-end bg-muted/20">
              <button
                onClick={() => router.push(`/dashboard/bookings/${booking.id}/edit`)}
                className="flex items-center gap-2 px-6 py-2.5 bg-primary text-primary-foreground hover:bg-primary/90 rounded-xl font-medium transition-colors shadow-sm"
              >
